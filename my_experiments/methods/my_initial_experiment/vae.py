...
from disentanglement_lib.methods.my_initial_experiment import gaussian_encoder_model
  ...
  def model_fn(self, features, labels, mode, params):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      summary_writer = tf.summary.FileWriter(
        logdir=os.path.join("tmp", "summary"),
        #flush_secs=60,
        )

      summary_hook = tf.estimator.SummarySaverHook(save_steps=100, 
                 summary_writer=summary_writer,
            summary_op=tf.summary.scalar("elbo", -elbo))
