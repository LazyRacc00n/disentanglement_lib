...
from config.no_shape import sweep as no_shape
...
from config.my_initial_experiment import sweep as my_initial_experiment
...
FLAGS = flags.FLAGS
flags.DEFINE_string("study", "no_shape",
                    "Name of the study.")
...
def prepare_gin(model_dir,
  ...
  prepare_arguments(model_dir, output_dir, overwrite)
  gin.clear_config()
...
def prepare_arguments(model_dir,
  ...
  with gin.unlock_config():
      gin.bind_parameter("my_dataset.name", gin_dict["my_dataset.name"].replace(
          "'", ""))
  ...
  with hub.eval_function_for_module(module_path) as f:
    ...
    numpy_data, _, numpy_classes = utils.generate_batch_factor_code(
      dataset, _representation_function, 10000,
      np.random.RandomState(random_seed), 1)
    
    print(os.path.join(model_dir, "train_representation.npy"))
    with open(os.path.join(model_dir, "train_representation.npy"), 'wb') as f:
      np.save(f, numpy_data)
    with open(os.path.join(model_dir, "train_classes.npy"), 'wb') as f:
      np.save(f, numpy_classes)
...
def main(unused_argv):
  ...
  #study = my_initial_experiment.MyInitialExperiment()
  study = no_shape.NoShape()
  ...
  for config in postprocess_config_files:
    ...
    prepare_gin(post_dir, metric_dir, FLAGS.overwrite, gin_eval_config, eval_bindings)
