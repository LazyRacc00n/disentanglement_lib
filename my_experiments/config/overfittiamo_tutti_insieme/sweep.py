...
def get_default_models():
  ...
  model_steps = h.fixed("model.training_steps", 150000)
...
class OverfittiamoTuttiInsieme(study.Study):
  ...
  def get_model_config(self, model_num=0):
    ...
    model_config_file = "config/overfittiamo_tutti_insieme/model_configs/shared.gin"
    return model_bindings, model_config_file

  def get_postprocess_config_files(self):
    """Returns postprocessing config files."""
    return ["config/overfittiamo_tutti_insieme/postprocess_configs/mean.gin", "config/my_initial_experiment/postprocess_configs/sampled.gin"]
  ...
  def get_eval_config_files(self):
    ...
    return [os.path.join(os.getcwd(), "config/overfittiamo_tutti_insieme/metric_configs", f) for f in files ]
