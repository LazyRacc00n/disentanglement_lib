...
class OnlyDisentangled(study.Study):
  ...
  def get_model_config(self, model_num=0):
    ...
    model_config_file = "config/only_disentangled/model_configs/shared.gin"
    return model_bindings, model_config_file

  def get_postprocess_config_files(self):
    """Returns postprocessing config files."""
    return ["config/only_disentangled/postprocess_configs/mean.gin", "config/only_disentangled/postprocess_configs/sampled.gin", "config/only_disentangled/postprocess_configs/mean_std.gin"]
  ...
  def get_eval_config_files(self):
    ...
    return [os.path.join(os.getcwd(), "config/only_disentangled/metric_configs", f) for f in files ]
