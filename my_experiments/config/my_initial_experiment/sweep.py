...
def get_default_models():
  ...
  model_fn = h.fixed("model.training_steps", 150000)
...
class MyInitialExperiment(study.Study):
  ...
  def get_model_config(self, model_num=0):
    ...
    model_config_file = "config/my_initial_experiment/model_configs/shared.gin"
    return model_bindings, model_config_file

  def get_postprocess_config_files(self):
    """Returns postprocessing config files."""
    return ["config/my_initial_experiment/postprocess_configs/mean.gin", "config/my_initial_experiment/postprocess_configs/sampled.gin", "config/my_initial_experiment/postprocess_configs/mean_std.gin"]
  ...
  def get_eval_config_files(self):
    ...
    return [os.path.join(os.getcwd(), "config/my_initial_experiment/metric_configs", f) for f in files ]
