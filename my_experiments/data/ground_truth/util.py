...
class SplitDiscreteStateSpaceOnlyHeart(SplitDiscreteStateSpace):
  """Only Heart shape allowed."""
  ...
  def sample_latent_factors(self, num, random_state):
    ...
    factors = np.zeros(
        shape=(num, len(self.latent_factor_indices)), dtype=np.int64)
    for pos, i in enumerate(self.latent_factor_indices):
      # factor shape is at index 1
      if i == 1:
         factors[:, pos] = np.ones(num) * 2 #random_state.randint(2, size=num)
      else:
         factors[:, pos] = self._sample_factor(i, num, random_state)
    return factors
