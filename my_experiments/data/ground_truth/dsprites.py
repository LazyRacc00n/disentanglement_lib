...
class DSprites(ground_truth_data.GroundTruthData):
  ...
  def sample_observations_from_factors(self, factors, random_state):
    images, classes = self.sample_observations_from_factors_no_color(factors, random_state)
  ...
  def sample_observations_from_factors_no_color(self, factors, random_state):
    ...
    return np.expand_dims(self.images[indices].astype(np.float32), axis=3), self.latents_classes[indices]

  def _sample_factor(self, i, num, random_state):
    return random_state.randint(self.factor_sizes[i], size=num)

class HeartDSprites(DSprites):
  """Heart DSprites.
  ...
  def __init__(self, latent_factor_indices=None):
    ...
    self.factor_bases = np.prod(self.factor_sizes) / np.cumprod(
        self.factor_sizes)
    self.state_space = util.SplitDiscreteStateSpaceOnlyHeart(self.factor_sizes,
                                                    self.latent_factor_indices)


  def sample_observations_from_factors(self, factors, random_state):
    images, classes = self.sample_observations_from_factors_heart(factors, random_state)
    return images, classes

  def sample_observations_from_factors_heart(self, factors, random_state): 
    """Sample a batch of observations X given a batch of factors Y.
       Return also the factor classes
    """
    all_factors = self.state_space.sample_all_factors(factors, random_state)
