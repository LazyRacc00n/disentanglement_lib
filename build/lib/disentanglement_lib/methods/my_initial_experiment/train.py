...
def train(model_dir,
  ...
  summary_writer = tf.compat.v1.summary.FileWriter(
        logdir=os.path.join(model_dir),
        flush_secs=180,
        filename_suffix='.txt')
  
  summary_hook = [tf.estimator.SummarySaverHook(save_steps=1000, 
                 output_dir="/train_summaries",
            summary_op=tf.summary.merge_all())
  ]
  # #output_dir=os.path.join(model_dir, "train_summaries"),
  # #summary_writer=summary_writer,

  # Do the actual training.
  history = tpu_estimator.train(
